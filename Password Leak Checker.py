...
user_input = input("Input your password/passwords separated by comma: ").split(", ")


# This is a function that fetches a list of already hacked/leaked passwords through an api request

def request_api_data(my_hashed_password):  # This parameter is the hashed version of our password
    url = 'https://api.pwnedpasswords.com/range/' + my_hashed_password  # We give it an argument containing the first five character of our hashed password
    url_response = requests.get(url)
    if url_response.status_code != 200:
        raise RuntimeError(f"Error fetching: {url_response.status_code}, Please check the api and try again")
    return url_response
...
def main(user_passwords):
    for password in user_passwords:
        ...
        result = int(result)
        if 0 < result <= 19:
            print(f"{password} was found {result} times, you probably might change it")
        elif result >= 20:
            print(f"{password} was found {result} times, you should definitely change it")
        else:
